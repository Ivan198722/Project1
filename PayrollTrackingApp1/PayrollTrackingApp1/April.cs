...
{
    public partial class April : Form
    {
        ...
        private void April_Load(object sender, EventArgs e)
        {
            // TODO: данная строка кода позволяет загрузить данные в таблицу "payrollTrackingDBDataSet4.April_2023". При необходимости она может быть перемещена или удалена.
            this.april_2023TableAdapter.Fill(this.payrollTrackingDBDataSet4.April_2023);
        ...
        class Salary : SalaryCalculator
        {
            internal override IQueryable<object> GetSalaryData(int id)
            {
                var query = from pos in _dbEntities.PositionsAndPays
                            ...
                            join month in _dbEntities.April_2023 on emp.Id equals month.IdEmployee
                            where month.Id == id
            ...
            internal override IQueryable<object> GetSalaryData_Month(int id)
            {
                ...
                var query = from month in _dbEntities.April_2023
                            where month.Id == id
                            select new
                            {
        ...
        private void add_button_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string query = "UPDATE April_2023 SET [1]=" + day1 + ",[1DailiSalary]=" +salary.WeekendDaySalary(day1, id) + ",[2]=" + day2 + ",[2DailiSalary]=" + salary.WeekendDaySalary(day2, id) + "," +
            ...
            try
            {
                string query1 = "UPDATE April_2023 SET MonthliSalary=" + salary.MonthSalary(id) + " where Id=" + id;
